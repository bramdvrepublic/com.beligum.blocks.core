...
public class ElementParser
{
    ...
    public static boolean isCanLayout(Element node) { return node.hasAttr(ParserConstants.CAN_LAYOUT);}
    ...
    public static boolean isPageTemplateRootNode(Element node) {
        return node.hasAttr(ParserConstants.PAGE_TEMPLATE_ATTR);
    }
    ...
    public static String getPropertyKey(String name, Set<String> usedKeys) {
        int count = 0;
        String keyName = name;
        while (usedKeys.contains(keyName)) {
            count++;
            keyName = name + "/" + count;
        }
        return keyName;
