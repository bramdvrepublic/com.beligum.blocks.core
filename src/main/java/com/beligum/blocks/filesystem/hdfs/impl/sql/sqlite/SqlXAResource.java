 ...
 */
public class SqlXAResource implements XAClosableResource
{
    /**
     ...
     */
    protected Map<Xid, Object> m_xidStateMap = new HashMap<>(1);
     ...
     */
    public SqlXAResource(Connection connection)
    {
    ...
    public int prepare(Xid xid) throws XAException
    {
        ...
        return SqlXAResource.XA_OK; // vote ready to commit, no way of verifying this with RDBMS
    }
    ...
    {
        if (!m_xidStateMap.containsKey(xid)) {
    ...
    /**
     * (b) Added this method to expose the db ref for
     * @return
     */
    ...
    {
        // Note that we close the database connection, just as we didn't know it was a pooled connection;
        // the ConnectionPoolManager should handle the API call and act accordingly (recycling or disposing).
        if (this.m_connection != null) {
            try {
                this.m_connection.close();
            }
            catch (SQLException e) {
                throw new IOException("Error while closing SQL connection", e);
            }
        }
    }
}