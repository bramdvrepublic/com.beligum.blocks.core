...
public class JsonLDWrapper implements BlocksStorable
{
    ...
    private void cache() {
        ...
        for (String key: context.keySet()) {
            if (context.get(key) instanceof String) {
                if (!expanded.containsKey(key)) context.put(key, expand(key, (String) context.get(key)));
    ...
    public Resource getMainResource(String language) {
        if (cachedFields == null || cachedResources == null) this.cache();
        return getResource(this.mainResource, language);
    }


    public Resource getResource(String id, String language) {

        return Resource.createResource(cachedResources.get(id), this, language);
    }
    @Override
