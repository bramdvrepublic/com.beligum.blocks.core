...
public class ResourceJsonLDSerializer extends JsonSerializer<Resource>
{
    ...
    private void printNode(JsonGenerator jgen, Node field, Locale locale) throws IOException
    {
        if (field.isIterable()) {
            ...
            StringNode stringNode = (StringNode)field;
            jgen.writeStartObject();
            jgen.writeFieldName("@value");
            jgen.writeString(stringNode.asString());
            if (stringNode.getLanguage() != Locale.ROOT) {
                jgen.writeFieldName("@language");
                jgen.writeString(stringNode.getLanguage().getLanguage());
            }
            jgen.writeEndObject();
